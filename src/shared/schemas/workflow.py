...
from src.shared.schemas.resource import ResourceReference
...
class CaptureDefinition(BaseModel):
    ...
    Flow: prompt -> user interaction -> vela_advance_workflow(step_output)
          -> capture validation -> ctx.elicit() for missing/structured -> save

    The `input` field controls the elicitation UI type used during
    the tool call when VELA validates and collects data via ctx.elicit().
    """
    key: str
...
class FetchDefinition(BaseModel):
    ...
    VELA acts as MCP client, calls mounted servers, and injects
